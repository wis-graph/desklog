...
'집중'에 대하여:
  ...
  주의: 자동화·AI 에이전트가 만든 합성 입력은 사람 입력과 구분되지 않는다(OS 한계).
  그런 앱은 note 로 표시해 둔다 — 예: note 캘린더 \"AI 자동화. 입력=합성, 사람 아님\".
  근본 해결은 사람이 앉아 있는지(재석/공석)를 카메라로 보는 것이다. 공석인데 입력이 있으면
  자동화다. 그 재석 감지는 desklog 밖의 별도 시스템 몫이다 — desklog 는 입력만 잰다.
";
